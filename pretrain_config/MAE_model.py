...
class MAE_Encoder(torch.nn.Module):
    def __init__(self,
        ...
        new_sequence_length = int(3840/patch_size) #3840 is the signals per channel     
...
class MAE_Decoder(torch.nn.Module):
    ...
    def forward(self, features, patch_indices):        
        ...
        mask_token_expanded = self.mask_token.expand(batch_size, channels, original_seq_length - seq_length, -1)  
        
        full_sequence = torch.zeros(batch_size, channels, original_seq_length, emb_dim, device = features.device)  
        final_mask = torch.zeros(batch_size, channels, original_seq_length, self.patch_size, device = features.device) 
                
        for batch_idx in range(batch_size):
            kept_signals = patch_indices[batch_idx]
            start_idx = kept_signals[0]
            end_idx = kept_signals[1]
            for channel in range(channels):
                mask = torch.ones(original_seq_length, dtype=torch.bool)
                unmasked_patch_indices = patch_indices[batch_idx][channel]
                mask[unmasked_patch_indices] = 0  #define mask as all of ones and then set the unmasked to zeroes so that masked are ones
                full_sequence[batch_idx, channel, mask, :] = mask_token_expanded[batch_idx, channel]
                final_mask[batch_idx, channel, mask, :] = 1   #masked regions are one
                channel_features = features[batch_idx, channel, :, :]
                full_sequence[batch_idx, channel, ~mask, :] = channel_features
        ...
        batch_size, channels, mask_seq_length, mask_emb_dim = final_mask.shape  
        final_mask = final_mask.reshape(batch_size, channels, mask_seq_length * mask_emb_dim) 
